...
public class CameraManager {
    ...
    CameraManager(double xPos, double yPos, double zPos, double xRot, double yRot, double zRot, int windowWidth, int windowLength, int fov){
        this.rx = xRot;
        this.ry = yRot;
        this.rz = zRot;
        this.x = xPos;
    ...
    void cull(Entity m){
        convertToViewSpace(m);
        ArrayList<int[]> validIndices = backFaceCulling(m);
        convertToClipSpace(m);
        finalizeTriangles(m, validIndices);
    }
    ArrayList<int[]> backFaceCulling(Entity m){
    ...
    void finalizeTriangles(Entity m, ArrayList<int[]> validIndices){
    ...
    double determineDirection(int[] tri, ArrayList<double[]> vtx) {
    ...
    ArrayList<Integer> clipTriangle(ArrayList<Integer> v, Entity m){
    ...
    boolean isInPlane(int plane, double[] v){
        switch (plane) {
            case 0:
                return ((v[0] + v[3]) >= 0);
            case 1:
    ...
    double[] calculateIntersection(double[] p1, double[] p2, int plane){
        double fp1;
        double fp2;
        double t;
        switch (plane) {
